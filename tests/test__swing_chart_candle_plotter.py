...
class TestSwingChartCandlePlotter:
    ...
    def test__detect_swing_extremes_across_data_set__2bar(self) -> None:
    ...
    def test__detect_swing_extremes_across_data_set__1bar(self) -> None:
        # Outside-bar should only have one swing extreme at the top
        # Down-bar should only have one swing extreme at the bottom
        data = {
            'date': pd.date_range(start='2023-01-01', periods=10, freq='D'),
            'high': [1, 2, 3, 4, 5, 4, 3, 2, 1, 2],
            'low': [0, 1, 2, 3, 2, 3, 2, 1, 0, 1],
        ...
        expected_data = {
            ...
            'close': [4.8, 4.8, 0.8],
            'volume': [500, 500, 100],
            'bar_type': [BarType.OUTSIDE_BAR, BarType.OUTSIDE_BAR, BarType.DOWN_BAR],
            'to_plot': ['low', 'high', 'low']
        }
        expected_df = pd.DataFrame(expected_data)
